...
ipcMain.handle("pathwarden:summarizePath", async (_, targetPath) => runPathwardenJsonScript("scripts/dev/summarize-path-json.ts", [targetPath]));
ipcMain.handle("pathwarden:searchPath", async (_, targetPath, extension, nameContains, minSizeBytes) =>
  runPathwardenJsonScript("scripts/dev/search-path-json.ts", [
    targetPath,
